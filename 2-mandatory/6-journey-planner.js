...
function getTransportModes(londonLocations) {
  transportMode = londonLocations.slice(1);
...
*/
function isAccessibleByTransportMode(arrOfTransportMode, strOfTransportMode) {
  return arrOfTransportMode.includes(strOfTransportMode);
}
...
function journeyPlanner(locations, transportMode) {
  ...
  return locations.filter((arr) => arr.includes(transportMode)).map((arr) => getLocationName(arr));
}
