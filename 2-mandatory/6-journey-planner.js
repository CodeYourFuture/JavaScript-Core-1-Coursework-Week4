...
function getLocationName(array) {
  let locationName = array.slice(0,1).toString();
  return locationName;
}
...
function journeyPlanner(locations, transportMode) {
 return locations.filter(item => isAccessibleByTransportMode(item,transportMode)).map(item => getLocationName(item));


}
journeyPlanner(["London Bridge","Greenwich",],'river boat')
