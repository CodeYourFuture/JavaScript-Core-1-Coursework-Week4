...
*/
function isAccessibleByTransportMode(locationElement, transportMode) {
  return locationElement.includes(transportMode);
}
...
function journeyPlanner(locations, transportMode) {
  let newlocation = locations.filter((locationElement) => {
    return isAccessibleByTransportMode(locationElement, transportMode);
  });
  let locationNames = newlocation.map(function getLocationName(
    locationElement
  ) {
    return locationElement[0];
  });
