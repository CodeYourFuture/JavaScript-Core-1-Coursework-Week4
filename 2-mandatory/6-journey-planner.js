...
function checkCodeIsThere(stringText) {
  let magicWord = "code";
  let containCode = stringText.includes(magicWord);
  if (containCode) {
...
function getTransportModes(location) {
  let transportType = location.slice(1); 
...
*/
function isAccessibleByTransportMode(modes, chosenMode) {
  return modes.includes(chosenMode); 
}
...
/*
  Implement the function getLocationName that
   ...
   - Returns the name of the location
      e.g: "Tower Bridge"
*/
function getLocationName(arrOfLocation) {
  let locationName = arrOfLocation[0]; 
  return locationName; 
}
...
function journeyPlanner(locations, transportMode) {
  return locations.filter((location) => {
    return isAccessibleByTransportMode(location, transportMode)}).map((accessibleLocation) =>{
