...
function checkCodeIsThere(stringText) {
  let magicWord = "code";
  //edit code below
  let codam = stringText.includes(magicWord);
  if (codam) {
...
*/
function isAccessibleByTransportMode(array, index) {
...
function journeyPlanner(locations, transportMode) {
  let holiday = locations.filter((journey) => {
    if (
      isAccessibleByTransportMode(journey) &&
      getTransportModes(journey) == transportMode
    ) {
      return getLocationName(journey);
    }
  });
  return holiday;
}
