...
*/
function getTransportModes(arrayOfTransports) {
  // const [, ...rest] = arrayOfTransports;
  // return rest;
  return arrayOfTransports.splice(1);
}
...
/*
  Implement the function getLocationName that
   ...
   - Returns the name of the location
      e.g: "Tower Bridge"
*/
function getLocationName(arrayOfTransport) {
  return arrayOfTransport[0];
}
...
function journeyPlanner(locations, transportMode) {
  return locations.filter(item => item.includes(transportMode)).map(item => getLocationName(item))
}
