...
/*
  Implement the function getLocationName that
   ...
   - Returns the name of the location
      e.g: "Tower Bridge"
*/
function getLocationName(locatTransportArr) {
  return locatTransportArr[0];
}
...
function journeyPlanner(locations, transportMode) {
  ...
  function isAvailableTransport(locationTransportArr) {
    return locationTransportArr.includes(transportMode);
  }

  let availableArr = locations.filter(isAvailableTransport);
  let myJourneyPlannerArr = availableArr.map((locTrans) =>
    getLocationName(locTrans)
  );
  return myJourneyPlannerArr;
}
