...
function checkCodeIsThere(stringText) {
  let index = stringText.match(/code/);
  if (index) {
    return index.index;
  } else {
    return "Not found";
...
*/
function isAccessibleByTransportMode(transports, transpMode) {
  return transports.includes(transpMode);
}
...
function getLocationName(locationAndTransports) {
  let locationAndTransportsCopy = [...locationAndTransports];
  return locationAndTransportsCopy.shift();
}
...
function journeyPlanner(locations, transportMode) {
  return locations.reduce((acc, value) => {
    if (value.includes(transportMode)) {
      acc.push(getLocationName(value));
    }
    return acc;
  }, []);
}
