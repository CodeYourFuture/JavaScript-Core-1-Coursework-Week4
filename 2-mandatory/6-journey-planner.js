...
*/
function isAccessibleByTransportMode(transPortModesArr, transportMode) {
  if (transPortModesArr.includes(transportMode)) {
    return true
  } else {
    return false
  }
...
/*
  Implement the function getLocationName that
   ...
   - Returns the name of the location
      e.g: "Tower Bridge"
*/
function getLocationName(locationAndTransportModesArr) {
  return locationAndTransportModesArr[0]
}
...
*/
function journeyPlanner(locationsArr, desiredTransportMode) {
  let accessibleLocations = [];
  for (let location of locationsArr) {
    
    if (location.includes(desiredTransportMode)) {
      accessibleLocations.push(location[0])
    }
  }
