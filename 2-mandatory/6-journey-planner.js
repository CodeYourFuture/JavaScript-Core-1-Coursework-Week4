...
*/
function getTransportModes(routFinder) {
  return routFinder.slice(1);
}
...
*/
function isAccessibleByTransportMode(routeArray, transportMode) {
  return routeArray.includes(transportMode);
}
...
/*
  Implement the function getLocationName that
   ...
   - Returns the name of the location
      e.g: "Tower Bridge"
*/
function getLocationName(routeArray) {
  return routeArray[0];
}
...
function journeyPlanner(locations, transportMode) {
  ...
  return locations
    .filter((x) => x.includes(transportMode))
    .map(getLocationName);
}
