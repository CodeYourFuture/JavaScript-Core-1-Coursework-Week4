...
function checkCodeIsThere(stringText) {
  ...
  if (stringText.search(magicWord)!=-1) {
    return stringText.search(magicWord);
  } else {
    return "Not found";
  }
}
...
let transportMode=array.slice(1,array.length);
...
function journeyPlanner(locations, transportMode) {
  let location=[];
  locations.map(array=>{
    if(array.slice(1,array.length).includes(transportMode)){
      location.push(array[0])
    }
  });
return location;
}
