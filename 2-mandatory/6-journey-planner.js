...
function getTransportModes(array) {
  let transportModesArr = array.slice(1, array.length);
  return transportModesArr;
}
...
function isAccessibleByTransportMode(array, mode) {
  return array.some((item) => item === mode);
}
...
function journeyPlanner(locations, transportMode) {
  ...
  let transportArr = locations
    .filter((x) => getTransportModes(x).some((y) => y === transportMode))
    .map((x) => x[0]);
  return transportArr;
}
