...
function getTransportModes(locationInfo) {
  const location = locationInfo.shift();
  return locationInfo;
}
...
function journeyPlanner(locations, transportMode) {
  const accessibleLocations = locations.filter((locationInfo) => {
    return isAccessibleByTransportMode(locationInfo, transportMode);
  });
  const locationNames = accessibleLocations.map((locationInfo) => {
    return getLocationName(locationInfo);
  });
