...
function findSafeOxygenLevel(arrOfO2Level) {

  let firstSafePlanet = arrOfO2Level.filter((level)=> level.includes("%"))
                                  
                        .map((level)=> Number(level.replace("%","")))
                        .find((level)=>19.5 <level && level<23.5);
             
     if(firstSafePlanet>0){
