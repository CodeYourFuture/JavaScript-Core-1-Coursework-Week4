...
function validatePasswords(passwords) {
  return passwords.map((val, index) => {
    if (val.length >= 5 &&
    containsUppercaseLetter(val) && containsLowercaseLetter(val) && containsNumber(val) && containsSymbol(val) && passwords.indexOf(val) === index) {
      return true
    } return false;
  })
