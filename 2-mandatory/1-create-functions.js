...
*/
function tidyUpString(names) {
  return names.map((name) => name.trim().replace(/\//g, '').toLowerCase()
)};
/*
Write a function that:
...
function remove(element, index ) {
  return element.slice(0, index).concat(element.slice(index + 1));
}
