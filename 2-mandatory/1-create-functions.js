...
function tidyUpString(arr) {
  let noWhiteSpaceArray = arr.map(value => value.trim());
  let noForwardSlashes = noWhiteSpaceArray.map(value => value.replace("/",""));
  let lowerCases = noForwardSlashes.map(value =>value.toLowerCase());
 return lowerCases
