...
*/
function first5(arrayIn) {
  return arrayIn.slice(0, 5);
}
...
function remove(arrayIn, pointer) {
  let newArr = [...arrayIn];
  newArr.splice(pointer, 1);
  return newArr;
}
...
function formatPercentage(numArray) {
  let percentages = numArray.map(fixPercentage);
  return percentages;
}
