...
function tidyUpString(arrayOfStrings) {
  return arrayOfStrings.map((itemsInString)=>itemsInString.trim().replace("/", "").toLowerCase());
};
...
function remove(array, index) {
  let aNewarray = array.slice();
  aNewarray.splice(index, 1);
  return aNewarray;
};
...
function formatPercentage(arrayOfNumbers) {
  ...
  }//my brain is hurting from trying to do this exercise. I cant figure it out. I'll try again.
