...
function tidyUpString(arrayOfStrings){
  return arrayOfStrings.map((string) => string.trim().replace("/", "").toLowerCase());
}
...
function formatPercentage(arrayOfNumbers) {
  return arrayOfNumbers.map((number) => number > 100 ? "100%" : Math.round((number + Number.EPSILON) * 100) / 100 + "%")
}
